...
import {sendMessage, viewMessage} from '../../api/userService';
function CommunicationTab({ projectId }) {
  ...
  const fetchMessages = async () => {
    try {
      ...
      const response = await viewMessage(projectId);
      ...
      if (response.data.success && response.data.data) {
        ...
        const transformedComments = response.data.data.map((message, index) => ({
          ...
          isStudent: message.senderId.role === 'student',
        ...
        {comments.length === 0 ? (
          ...
          <>
            {comments.map((comment) => (
              <div
                key={comment.id}
                className={`flex ${comment.isStudent ? 'justify-end' : 'justify-start'}`}
              >
                <div className="flex items-start space-x-2 max-w-lg">
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mb-1">
                      <span
                        className={`text-xs font-medium ${
                          comment.isStudent ? 'text-blue-100' : 'text-gray-600'
                        }`}
                      >
                      ...
                      {comment.isEdited && (
                        <span
                          className={`text-xs ml-2 ${
                            comment.isStudent ? 'text-blue-200' : 'text-gray-500'
                          }`}
                        >
                          edited
                        </span>
                    ...
                    {/* Message content */}
                    <p className={`text-sm leading-relaxed ${comment.isStudent ? 'text-white' : 'text-gray-900'}`}>
                      {comment.message}
                    </p>
